...
const getAchievements = item => {
  return (
    <>
      <h4 className="curriculum-vitae-timeline-item-desc-heading font-weight-bold">
        Achievements:
      </h4>
      <p>{item.ACHIEVEMENTS}</p>
      <ul>
        {item.ACHIEVEMENTS_POINTS &&
          item.ACHIEVEMENTS_POINTS.length &&
          item.ACHIEVEMENTS_POINTS.map(item => <li>{item}</li>)}
      </ul>
    </>
  )
}
  ...
  return (
    <>
      ...
      <ul className="list-inline">
        {item.TECHNOLOGIES_USED.map(item => (
...
const getExperiences = EXPERIENCES => {
  return EXPERIENCES.map(item => (
    <section
      className="curriculum-vitae-timeline-item position-relative pb-5"
      key={`${item.DURATION}${item.COMPANY_NAME}`}
    >
      <div className="curriculum-vitae-timeline-item-header mb-2">
        <div className="d-flex flex-column flex-md-row">
          <h3 className="curriculum-vitae-position-title font-weight-bold mb-1">
            {item.ROLE}
          </h3>
          <div className="curriculum-vitae-company-name ml-auto">
            {item.COMPANY_NAME}
          </div>
        </div>
      ...
      <div className="curriculum-vitae-timeline-item-desc">
        ...
        {item.DESCRIPTION_POINTS &&
          item.DESCRIPTION_POINTS.length &&
          getDescriptionPoints(item.DESCRIPTION_POINTS)}
        {item.ACHIEVEMENTS && getAchievements(item)}
        {item.TECHNOLOGIES_USED &&
          item.TECHNOLOGIES_USED.length &&
          getTechnologyList(item)}
      </div>
    </section>
  ))
}
