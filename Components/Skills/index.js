...
const getSkill = SKILLS => {
  return SKILLS.map(item => (
    <div className="curriculum-vitae-skill-item" key={item.HEADING}>
      <h4 className="curriculum-vitae-skills-cat font-weight-bold">
        {item.HEADING}
      </h4>
      <ul className="list-unstyled mb-4">{getSkillList(item.SKILLS_LIST)}</ul>
    </div>
  ))
}

const getOtherSkillList = OTHERS_SKILLS => {
  return OTHERS_SKILLS.map(item => (
  ...
  return (
    <section className="curriculum-vitae-section skills-section mb-5">
      ...
      <div className="curriculum-vitae-section-content">
        {getSkill(SKILLS)}
        {OTHERS_SKILLS && OTHERS_SKILLS.length && (
          <div className="curriculum-vitae-skill-item">
            <h4 className="curriculum-vitae-skills-cat font-weight-bold">
              Others
            </h4>
            <ul className="list-inline">{getOtherSkillList(OTHERS_SKILLS)}</ul>
          </div>
        )}
      </div>
    </section>
